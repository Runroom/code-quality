...
function printOutcome(
  row: OutcomeRow,
  deps: CliDeps,
  options: { all: boolean; idWidth: number; toolWidth: number },
): void {
  for (const line of renderCheck(row, {
    all: options.all,
    githubActions: deps.env.GITHUB_ACTIONS === "true",
    idWidth: options.idWidth,
    toolWidth: options.toolWidth,
  }, deps.style)) deps.stdout(`${line}\n`);
  const { outcome } = row;
...
function printNoticesOrReject(config: ResolvedConfig, deps: CliDeps): boolean {
  for (const notice of config.notices) deps.stdout(`${renderNotice(notice, deps.style)}\n`);
...
function runtimeNoticeSink(deps: CliDeps): (message: string) => void {
  ...
  return (message) => {
    ...
    emitted.add(key);
    deps.stdout(`${renderNotice(message, deps.style)}\n`);
  };
}
...
function keepExistingBaseline(adapter: CheckAdapter, root: string, deps: CliDeps): boolean {
  ...
  deps.stdout(` ${deps.style.dim(`${GLYPH.bullet} Kept existing ${sanitizeLine(file)}`)}\n`);
  return true;
}
...
function printSkippedAndSummary(
  rows: readonly OutcomeRow[],
  skipped: ReturnType<typeof selectAdapters>["skipped"],
  idWidth: number,
  deps: CliDeps,
): void {
  for (const entry of skipped) deps.stdout(`${renderSkipped(entry, idWidth, deps.style)}\n`);
  for (const line of renderSummary(rows, skipped, deps.style)) deps.stdout(`${line}\n`);
}
...
export async function checkCommand(
  ...
  const idsForWidth = [
    ...selection.adapters.map((adapter) => adapter.id),
    ...selection.failed.map((entry) => entry.id),
    ...selection.skipped.map((entry) => entry.id),
  ];
  const idWidth = Math.max(...idsForWidth.map((id) => id.length), 0);
  const tools = selection.adapters.map((adapter) => `${adapter.tool.bin} ${adapter.tool.version}`);
  const toolWidth = Math.max(...tools.map((tool) => tool.length), 0);
  if (options.header !== false) {
    deps.stdout(`${renderHeader({
      version: CLI_VERSION,
      languages: config.languages,
      checks: selection.adapters.length + selection.failed.length,
    }, deps.style)}\n`);
    deps.stdout("\n");
  }
  ...
  for (const entry of selection.failed) {
    ...
    rows.push(row);
    printOutcome(row, deps, { all: options.all === true, idWidth, toolWidth });
  }
  ...
  printSkippedAndSummary(rows, selection.skipped, idWidth, deps);
