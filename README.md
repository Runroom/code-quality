...
A release tag also publishes the npm launcher from `launcher/` through npm trusted publishing (OIDC). The very first version must be published by hand from `launcher/` (`pnpm build`, then `npm publish --access public`), after which the trusted publisher for repository `Runroom/code-quality` and workflow `release.yml` must be configured on npmjs.com. A version bump must update root `package.json`, `launcher/package.json`, and the `.version()` literal in `src/cli/program.ts` together.
